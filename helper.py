...
class BatchGraphNuc(nn.Module):
    ...
    def forward(self, s_nuc, v_nuc, coord_elec, coord_nuc):
        ...
        data_list = [
            Bipartite(radius(e, n, 5.0), e, n, sn, vn, n_elec)
...
class BatchGraphElec(nn.Module):
    ...
    def forward(self, s, v, rs):
        ...
        batch_edge_index = radius_graph(
            data.r, r=self.cut_off, batch=data.batch, loop=False
        )
...
class BackflowPaiNN(nn.Module):
    def __init__(self, embedding_dim, n_backflows, num_electrons):
        super().__init__()

        self.net = nn.Sequential(
            Linear(embedding_dim, embedding_dim), Linear(embedding_dim, 1)
        )
